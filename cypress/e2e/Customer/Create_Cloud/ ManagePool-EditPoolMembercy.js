describe(' Edit Pool Member', () => {
    ...
    it('Action success', () => {
        ...
        cy.get(':nth-child(2) > .px-0').click(); //Pool member Tab
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get('[name="0IpAddress"]').type('15.255.123.109');
        cy.get('[name="0Port"]').type('61');
    ...
    it('Validation (User did not specify Name.  The system display alert message “Please input data”)', () => {
        ...
        cy.get(':nth-child(2) > .px-0').click(); //Pool member Tab
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get('[name="0IpAddress"]').clear();
        cy.get('[name="0Port"]').clear();
        cy.get('[name="0Weight"]').clear();
        cy.get('[type="submit"]').click();
    ...
    it('Validation (User entered an Invalid format IP address.  The system dispaly alert massege “Invalid IP Address”)', () => {
        ...
        cy.get(':nth-child(2) > .px-0').click(); //Pool member Tab
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get('[name="0IpAddress"]').clear().type('15');
        cy.get('[name="0Port"]').clear().type('61');
    ...
    it('Validation (User entered a Port more than 65535.  The system dispaly alert massege “Please input data in range 1-65535”)', () => {
        ...
        cy.get(':nth-child(2) > .px-0').click(); //Pool member Tab
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get('[name="0IpAddress"]').clear().type('15.255.123.109');
        cy.get('[name="0Port"]').clear().type('65536');
        cy.get('[name="0Weight"]').clear().type('1');
        cy.get('[type="submit"]').click();
        cy.get('.text-danger').contains('Please input data in range 1-65535');
        cy.wait(700);
    ...
    it('Validation (User entered more or less ports than the system-defined range.  The system dispaly alert massege “Please input data in range 0-256”)', () => {
        ...
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get('[name="0IpAddress"]').clear().type('15.255.123.109');
        cy.get('[name="0Port"]').clear().type('60');
        cy.get('[name="0Weight"]').clear().type('-1');
        cy.get('[type="submit"]').click();
    ...
    it('Usiribities (User click icon add button in the Instance Hostname column.  The system display IP Address.)', () => {
        ...
        cy.get(':nth-child(2) > .px-0').click(); //Pool member Tab
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get(':nth-child(2) > .text-center > .app_renderer_common_button__common-button-style > .btn').click();
        cy.get('.ant-table-tbody > :nth-child(3) > :nth-child(1)').should('be.visible');
        cy.wait(700);
    ...
    it('Usiribities (User searches for Private Network entry by Instance Hostname.  The system displays the searched list.)', () => {
        ...
        cy.get(':nth-child(2) > .px-0').click(); //Pool member Tab
        cy.get(':nth-child(2) > .app_renderer_common_button__common-button-style > .btn').invoke('removeAttr', 'target').click(); //Edit Pool member
        cy.get('#search').type('Wara-Test-2');
        cy.wait(700);
